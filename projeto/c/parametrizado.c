// Compilando:
// mpicc parametrizado.c -Wall -O3 $(apr-1-config --cflags --cppflags --includes --link-ld) -o bin
...
int main(int argc, char ** argv){
    ...
    for (intnative_t depth = minimum_Tree_Depth; depth <= maximum_Tree_Depth; depth += 2) {
        ...
        intnative_t partialChecksum = 0;
        intnative_t iterations = 1 << (maximum_Tree_Depth - depth + minimum_Tree_Depth);

        apr_pool_t* execPool;
        apr_pool_create_unmanaged(&execPool);
        // Tentar parametrizar esse laço!!!
        // Atualmente só funciona pra 2 threads
        if (comm_sz % 2 == 0){
            for (intnative_t i = 1; i <= iterations; i += comm_sz) {
        ...
        if (my_rank == 0) {
            ...
            for (int core = 1; core < comm_sz; core++) {
                MPI_Recv(&partialChecksum, 1, MPI_LONG, core, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                totalChecksum = totalChecksum + partialChecksum;
            }
        } else {
            MPI_Send(&totalChecksum, 1, MPI_LONG, 0, 0, MPI_COMM_WORLD);
        }
